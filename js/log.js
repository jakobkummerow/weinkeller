...
(function (ChangeDirection) {
    ChangeDirection[ChangeDirection["kAdding"] = 0] = "kAdding";
    ChangeDirection[ChangeDirection["kRemoving"] = 1] = "kRemoving";
    ChangeDirection[ChangeDirection["kUnknown"] = 2] = "kUnknown";
...
class LogTR {
    constructor(log) {
        ...
        this.reason_td = document.createElement('td');
        this.reason_string = document.createElement('span');
        this.select = document.createElement('select');
        this.direction = ChangeDirection.kUnknown;
    ...
    update() {
        ...
        if (direction !== this.direction) {
            ...
            for (let i in kSideLang.reasons) {
                let i_num = Number(i);
                if (i_num !== data.reason && !IsValidReasonFor(i_num, delta))
                    continue;
                let option = AddC(s, 'option');
                option.value = i;
                AddT(option, FormatReason(i_num));
                if (i_num === data.reason)
...
class LogUI {
    constructor(data) {
        ...
        this.count = document.createElement('select');
        this.log_data = [];
        this.sorter = (a, b) => {
            let a_date = a.date;
            let b_date = b.date;
